function [Y, placed] = generate_clean_micrograph_2d_discrete_rots_projection_matrix(x, W, N, m, rotations, PR_matrix, L, seed)
...
for k = 1 : placed
            projection = real(reshape(PR_matrix( :, :, randi(K), 1) * x, [L, L]));
%         I(candidate(1):candidate(1)+L-1, ...
%             candidate(2):candidate(2)+L-1) = projection;
    Y(locations(k, 1) + rangeL, locations(k, 2) + rangeL) = projection;
