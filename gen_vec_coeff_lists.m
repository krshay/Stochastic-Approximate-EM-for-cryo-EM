function [ell_list, m_list, s_list, p_inds, n_inds] = gen_vec_coeff_lists(ell_max, s_lens)
...
s_list = [];
for elll=0:ell_max
    ell_list = [ell_list; elll*ones(s_lens(elll+1)*(2*elll+1), 1)];
    m_mat = repmat(-elll:elll, s_lens(elll+1), 1);
    m_list = [m_list; m_mat(:)];
    s_mat = repmat((1:s_lens(elll+1)).', 1, 2*elll+1);
    s_list = [s_list; s_mat(:)];
end
...
for ii = 1:length(p_inds)
    n_inds(ii) = find(ell_list == ell_list(p_inds(ii)) & m_list == -m_list(p_inds(ii)) & s_list == s_list(p_inds(ii)));
end
end
