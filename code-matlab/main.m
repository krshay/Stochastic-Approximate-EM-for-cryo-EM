...
for i=[1] %, 7, 9, 11, 12, 14, 15, 16, 17, 18, 19, 20]
    ...
    % end
    sigma2 = 0; % mean(noise_stack.^2, "all");
...
PR_matrix_1 = PR_matrix_all_rotations_all_shifts( :, :, 1, 1);
proj1 = reshape(real(PR_matrix_1 * x_init_vec), L, L);
figure; imagesc(proj1)

proj2 = CTZ(vol_project(vol_init_trunc, rotations( :, :, 1)), shifts(1, :), L);
figure; imagesc(proj2)
...
for s=1:size(shifts, 1)
    for k=1:NUM_ROTATIONS
        PINV_PR_matrix_all_rotations_all_shifts( :, :, k, s) = pinv(PR_matrix_all_rotations_all_shifts( :, :, k, s));
    end
end
...
x_est = EM_stochastic_parallel_ls(patches, x_init, rho_init, ...
    L, rotations, sigma2, jball, Psilms, ell_max, s_lens, n_list, ...
    lms_list, shifts, psi_lsNn, STOCHASTIC_FACTOR, gs, NUM_ITERS, ls_A_dagger_patch);
