...
Nd = 40;
patches_clean = zeros(Nd, L, L);
pl_rot_curr_reshaped = zeros(1, 1, NUM_ROTATIONS, NUM_SHIFTS, Nd);
for p=1:Nd
    ROT = randi([1, NUM_ROTATIONS]);
    SHIFT = randi([1, NUM_SHIFTS]);
    patches_clean(p, :, :) = real(reshape(PR_matrix_all_rotations_all_shifts( :, :, ROT, SHIFT) * x_true_vec, [L, L]));
    pl_rot_curr_reshaped( :, :, ROT, SHIFT, p) = 1;
end
patches = patches_clean + normrnd(0, sigma, size(patches_clean));
...
x_est = EM_stochastic_parallel_ls_test(patches, x_init, rho_init, ...
    L, rotations, sigma2, jball, Psilms, ell_max, s_lens, n_list, ...
    lms_list, shifts, psi_lsNn, STOCHASTIC_FACTOR, gs, NUM_ITERS, PR_matrix_all_rotations_all_shifts, pl_rot_curr_reshaped);
