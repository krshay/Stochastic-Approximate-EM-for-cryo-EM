function [x_updated, A, y] = x_step_parallel_all(Ls, rotations, ...
    ell_max, s_lens, n_list, length_x, lms_list, psipsi_cell, Ipsis_cell, pl_rot_curr_1, pl_rot_curr_2, pl_rot_curr_3)
...
[yaw, pitch, roll] = dcm2angle(rotations( :, :, 1:50), 'ZYZ');
roll = roll + pi/2;
...
[yaw, pitch, roll] = dcm2angle(rotations( :, :,51:101), 'ZYZ');
roll = roll + pi/2;
omegas = [yaw, pitch, roll];
parfor l=1:size(Ls, 1)
    psipsi_l = psipsi_cell{l};
...
[yaw, pitch, roll] = dcm2angle(rotations( :, :,102:152), 'ZYZ');
roll = roll + pi/2;
omegas = [yaw, pitch, roll];
parfor l=1:size(Ls, 1)
    psipsi_l = psipsi_cell{l};
