function [x_curr, rho_curr] = EM_3d_parallel_stochastic_noprecomputation_Ipsis(patches_all, x_init, rho_init, ...
    L, rotations, sigma2, jball, Psilms, ell_max, s_lens, n_list, lms_list, ...
    vol_true, Ls, Ls_sampled, psi_lsNn, STOCHASTIC_FACTOR, gs)
...
while true
    ...
    tic
    Ipsis_cell_sampled = preprocess_Ipsi(L, Ls_sampled, psi_lsNn, ell_max, s_lens, n_list, patches_all_sampled);
    toc
    ...
    tic;
    [x_updated, A, y] = update_x(Ls_sampled, rotations, ell_max, ...
        s_lens, length_x, lms_list, gs, Ipsis_cell_sampled, pl_rot_curr_cell);
    toc

    %     tic;
    %     [x_updated1, A1, y1] = x_step_parallel_all_for_stochastic(Ls_sampled, rotations, ell_max, ...
